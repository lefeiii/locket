...
create table if not exists public.stories (
  ...
  title text not null,
  category text not null check (category in ('Crush', 'Friendship', 'School', 'Family', 'AITA', 'Update', 'Cheating')),
  body text not null,
  is_update boolean not null default false,
  previous_story_reference text,
  story_arc_id text,
  arc_title text,
  ...
  has_active_poll boolean not null default false,
...
create table if not exists public.follows (
  ...
  created_at timestamptz not null default now(),
  unique (follower_name, followed_name)
);
...
create table if not exists public.category_subscriptions (
  ...
  created_at timestamptz not null default now(),
  unique (subscriber_name, category)
);
...
create policy "Anyone can anonymously create stories"
  on public.stories for insert
  with check (true);

create policy "Anyone can follow anonymous posters"
...
create policy "Anyone can create story subscriptions"
  on public.story_subscriptions for insert
  with check (true);

create policy "Anyone can read story subscriptions"
  on public.story_subscriptions for select
  using (true);

create policy "Anyone can create category subscriptions"
  on public.category_subscriptions for insert
  with check (true);

create policy "Anyone can read category subscriptions"
  on public.category_subscriptions for select
  using (true);
...
create policy "Anyone can anonymously create comments"
...
create policy "Anyone can read story polls"
  on public.story_polls for select
  using (true);

create policy "Anyone can create story polls"
...
create policy "Anyone can vote once per device"
...
create policy "Anyone can read in-app notifications"
  on public.in_app_notifications for select
  using (true);

create policy "Anyone can create in-app notifications"
  on public.in_app_notifications for insert
  with check (true);

create policy "Anyone can read seasonal wrapped"
  on public.seasonal_wrapped for select
  using (true);

create policy "Anyone can create seasonal wrapped"
  on public.seasonal_wrapped for insert
  with check (true);

create policy "Anyone can read persona badges"
  on public.persona_badges for select
  using (true);

create index if not exists stories_created_at_idx on public.stories(created_at desc);
create index if not exists stories_arc_idx on public.stories(story_arc_id, part_number);
create index if not exists stories_emotional_feed_idx on public.stories(is_resolved, created_at desc);
...
create index if not exists follows_followed_name_idx on public.follows(followed_name);
create index if not exists story_subscriptions_arc_idx on public.story_subscriptions(story_arc_id);
create index if not exists notifications_recipient_idx on public.notifications(recipient_name, created_at desc);
